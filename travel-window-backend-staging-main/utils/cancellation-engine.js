...
function calculateCancellationScenario(p) {
  ...
  if (isMC) {
    ...
    if (isChargesMode) {
      ...
      supplierWillReturn  = round(baseOurCost - (airlineDeducted + autoSupplierCancCharge));
    ...
    if (isChargesMode) {
      ...
      airlineDeducted     = acc;
      totalCharges        = round(airlineDeducted + totalSupplierTook + currentMargin);
      supplierWillReturn  = round(paidAmount - (airlineDeducted + autoSupplierCancCharge));
    ...
    if (isChargesMode) {
      scenario            = '5A';
      airlineDeducted     = acc;
      totalCharges        = round(totalSupplierTook + acc + currentMargin);
      ...
      scenario            = '5B';
      airlineDeducted     = round(paidAmount - ara);
      totalCharges        = round(totalSupplierTook + airlineDeducted + currentMargin);
      supplierWillReturn  = round(ara);
      upfrontNeeded       = currentMargin;
      refundCommittedToClient = supplierWillReturn;
    }
    refundableAmount = refundCommittedToClient;
